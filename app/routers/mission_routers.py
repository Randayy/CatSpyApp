...
def get_list_of_missions(db: Session = Depends(get_db)) -> list:
    service = MissionService(db)
...
def change_notes_of_target(
    new_notes: str, mission_id: int, target_id: int, db: Session = Depends(get_db)
) -> dict:
    service = MissionService(db)
    return service.update_notes_of_target(mission_id, target_id, new_notes)


@mission_router.get("/{mission_id}/complete_mission")
